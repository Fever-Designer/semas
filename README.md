...
Run `database/migration_004.sql` once against your existing database. **This drops the `polls`,
`poll_options`, and `poll_votes` tables from increment 10** — back them up first if you need that
history; Polls & Surveys was a product decision to replace with Class Attendance, not an additive
increment.
...
**Class Attendance (replaces Polls & Surveys):**
...
**Analytics Dashboard (Administrator, Dean, HOD, Lecturer):**
- `public/analytics/dashboard.php` + `public/api/analytics-data.php` — Chart.js charts scoped per
  role (Administrator: event + class attendance trends, users by role, announcements by category;
  Dean: university-wide student status + event attendance trend; HOD: department class-attendance
  breakdown + student status + sessions-by-module; Lecturer: their own class-attendance breakdown +
  sessions trend). The page polls the JSON endpoint every 15 seconds for a "live dashboard" feel
  without needing WebSockets/SSE.
...
charts (Chart.js, polling `api/analytics-data.php` every 15s) plus the stats/quick-links/lists that
used to be spread across a plain dashboard and a separate Analytics Dashboard are now in one page.
`public/analytics/dashboard.php` is kept only as a redirect to `/dashboard.php` so old links don't
404. All the `/dashboard.php` redirects already hardcoded in `auth/login.php`, `auth/login-otp.php`,
...
roles: Events, AI Notification Generator, Participants, Event QR Codes, Scan/Mark Attendance,
...
- `public/hod/departments.php` (new) — create/edit departments and faculty assignment, with a quick
  student/lecturer/module count per department.
...
- `hod/departments.php` lets an HOD create/rename departments and reassign their faculty, but doesn't
...
- `includes/NotificationGenerator.php::publish()` (the AI Notification Generator) was quietly
  bypassing both the sender snapshot *and* would have bypassed recipient tracking — it now routes
  through `Announcement::create()` like every other send path, closing that gap.
...
- `hod/departments.php` is deleted; department CRUD moved to `admin/departments.php`
