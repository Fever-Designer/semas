...
$modStmt = $db->prepare('SELECT m.*, d.department_name FROM modules m LEFT JOIN departments d ON d.department_id = m.department_id WHERE m.module_id = :id AND m.lecturer_id = :lec');
$modStmt->execute(['id' => $moduleId, 'lec' => $lecturer['lecturer_id'] ?? 0]);
$module = $modStmt->fetch();
if (!$module) { http_response_code(404); die('Module not found or not assigned to you.'); }
