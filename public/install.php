...
$existingAdminCount = (int) $db->query(
    "SELECT COUNT(*) FROM users u JOIN roles r ON r.role_id=u.role_id WHERE r.role_name='Principal'"
)->fetchColumn();
...
if ($existingAdminCount > 0) {
    $error = 'An Principal account already exists. For security, delete this file (public/install.php) now.';
    ...
    if ($fullName === '' || $email === '' || strlen($password) < 8) {
        ...
        $roleId = (int) $db->query("SELECT role_id FROM roles WHERE role_name='Principal'")->fetchColumn();
        $db->prepare(
...
?>
    ...
    <?php if ($done): ?>
      <div class="alert alert-success small">
        Principal account created. <strong>Now delete public/install.php from the server</strong> —
    ...
    <?php else: ?>
      <p class="text-muted small">Create the first Principal account for this installation.</p>
      <form method="post">
        <?= csrf_field() ?>
        ...
        <button class="btn btn-semas w-100">Create Principal Account</button>
      </form>
    <?php endif; ?>
